    ...
    self.registration.pushManager
      ...
      .then(function (subscription) {
        var notificationsPath =
          `${NEXT_PUBLIC_BASE_URL}subscribe` +
            ...
            if (notification.actions && notification.actions.length) {
              ...
              for (
                ...
                if (notification.actions[actionIndex].icon) {
                  notificationOptions.actions[actionIndex].icon =
                    notification.actions[actionIndex].icon;
                }
              }
            }
      ...
      .catch(function (err) {
        console.error(
          "An error occurred while processing the push event.",
          err
        );
      })
  );
...
//   var targetUrl =
//     "https://pushpad.xyz/notifications/" + event.notification.tag + "/redirect";
//   if (event.action) {
...
//     self.notificationActions.hasOwnProperty(event.action)
//   ) {
//     event.waitUntil(
//       Promise.all([
//         fetch(targetUrl, {
...
//         }),
//         self.notificationActions[event.action](
//           event.notification.data.custom
//         ).catch(function (err) {
//           console.log(
//             "A custom action has been invoked but it has raised an exception: " +
//               err
//           );
//         }),
//       ])
...
self.addEventListener("pushsubscriptionchange", function (event) {
  event.waitUntil(
    fetch(`${process.env.NEXT_PUBLIC_BASE_URL}subscribe`, {
      method: "POST",
